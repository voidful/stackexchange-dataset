    ...
    def extract(self):
        if self.name == "all":
            for k in self.sites:
                ...
                # archive.extractall()
                # archive.close()
                command = "py7zr x {}/dumps/{} {}/dumps/{}".format(curr_dir, self.sites[k]["download"].replace(
                    "https://archive.org/download/stackexchange/", ""), curr_dir, k)
                print(command)
                if os.system(command):
            ...
            # archive.extractall()
            # archive.close()
            command = "py7zr x {}/dumps/{} {}/dumps/{}".format(curr_dir, self.sites[self.name]["download"].replace(
                "https://archive.org/download/stackexchange/", ""), curr_dir, self.name)
            print(command)
            if os.system(command):
